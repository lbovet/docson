...
var _jsonpointer = _interopRequireDefault(require("jsonpointer.js"));
...
function get_document(url) {
  if (!schemaDocuments[url]) {
    schemaDocuments[url] = _jquery.default.get(url).then(function (content) {
...
Handlebars.registerHelper('desc', function (schema) {
  ...
  if (_marked.default) {
    ...
    return new Handlebars.SafeString((0, _marked.default)(text));
  } else {
    return text;
  }
});
...
Handlebars.registerHelper('boxes', function (options) {
  ...
  _jquery.default.each(boxes[boxes.length - 1], function (k, box) {
    box.__boxId = k + 1;
...
var resolvePointerRef = function (ref) {
  ...
  try {
    return _jsonpointer.default.get(stack[1], ref);
  } catch (e) {
    console.log(e);
    return null;
  }
...
function renderSchema(schema) {
  if (stack.indexOf(schema) == -1) {
    ...
    return new Handlebars.SafeString(boxTemplate({
...
Handlebars.registerHelper('signature', function (schema, keyword, schemas) {
  ...
  return new Handlebars.SafeString(signatureTemplate({
    schema: schema,
    keyword: keyword,
    schemas: schemas
  }));
...
function init() {
  _jquery.default.get(docson.templateBaseUrl + "/box.html").done(function (content) {
    source = content;
    boxTemplate = Handlebars.compile(source);

    _jquery.default.get(docson.templateBaseUrl + "/signature.html").done(function (content) {
      source = content;
...
docson.doc = function (element, schema, ref, baseUrl) {
  ...
  var throttled_render = _lodash.default.throttle(() => renderBox(), 1000);
  ...
  ready.then(function () {
    ...
    var get_ref = function (uri) {
      return new Promise(resolve => {
        get_document(uri.clone().hash('').toString()).then(function (schema) {
          refs[uri.toString()] = uri.hash() ? _jsonpointer.default.get(schema, uri.hash()) : schema;
        }).then(function () {
          debug('get_ref');
          resolve();
        });
    ...
    renderBox = function () {
      ...
      if (ref) {
        ref = ref[0] !== '/' ? '/' + ref : ref;
        target = _jsonpointer.default.get(schema, ref);
        stack.push(schema);
      }
      ...
      element.find(".box-title").each(function () {
        var ref = (0, _jquery.default)(this).attr("ref");

        if (ref) {
          if (window.location.href.indexOf("docson/index.html") > -1) {
            (0, _jquery.default)(this).find(".box-name").css("cursor", "pointer").attr("title", "Open in new window").hover(function () {
              (0, _jquery.default)(this).addClass('link');
            }, function () {
              (0, _jquery.default)(this).removeClass('link');
            }).click(function () {
      ...
      element.find(".box").mouseenter(function () {
        (0, _jquery.default)(this).children(".source-button").fadeIn(300);
        (0, _jquery.default)(this).children(".box-body").children(".expand-button").fadeIn(300);
      });
      element.find(".box").mouseleave(function () {
        (0, _jquery.default)(this).children(".source-button").fadeOut(300);
        (0, _jquery.default)(this).children(".box-body").children(".expand-button").fadeOut(300);
      });
      element.find(".signature-type-expandable").click(function () {
        var boxId = (0, _jquery.default)(this).attr("boxid");
        (0, _jquery.default)(this).toggleClass("signature-type-expanded");
        (0, _jquery.default)(this).parent().parent().parent().children(".signature-box-container").children("[boxid='" + boxId + "']").toggle(resizeHandler ? 0 : 300);
        resized();
      });
      element.find(".expand-button").click(function () {
        if ((0, _jquery.default)(this).attr("expanded")) {
          (0, _jquery.default)(this).parent().parent().find(".expand-button").html(" + ").attr("title", "Expand all");
          (0, _jquery.default)(this).parent().parent().find(".signature-type-expandable").removeClass("signature-type-expanded");
          (0, _jquery.default)(this).parent().parent().find(".box-container").hide(resizeHandler ? 0 : 300);
          (0, _jquery.default)(this).parent().parent().find(".expand-button").removeAttr("expanded");
          resized();
        } else {
          (0, _jquery.default)(this).parent().parent().find(".expand-button").html(" - ").attr("title", "Collapse all");
          (0, _jquery.default)(this).parent().parent().find(".signature-type-expandable").addClass("signature-type-expanded");
          (0, _jquery.default)(this).parent().parent().find(".box-container").show(resizeHandler ? 0 : 300);
          (0, _jquery.default)(this).parent().parent().find(".expand-button").attr("expanded", true);
          resized();
        }
      });
      element.find(".source-button").click(function () {
        (0, _jquery.default)(this).parent().children(".box-body").toggle();
        (0, _jquery.default)(this).parent().children(".source").toggle();
        resized();
      });
    };
    ...
    var resolveSchemaRef = function (parentObject, item, baseUrl) {
      ...
      return new Promise((resolve, reject) => {
        ...
        if (typeof baseUrl === 'string') {
          baseUrl = new _urijs.default(baseUrl);
        }

        var uri = new _urijs.default(item); // ah, it's a relative url
        ...
        uri.normalize(); // use the normalized uri
    ...
    resolveRefsReentrant = function (schema, relPath) {
      ...
      if (relPath === undefined) {
        relPath = new _urijs.default(baseUrl);
      }

      let p = [];
      (0, _traverse.default)(schema).forEach(function (item) {
